...
const ArrayListActor = @import("../array_list_actor.zig").ArrayListActor;
const StructWithArrayList = @import("../array_list_actor.zig").StructWithArrayList;

pub const ArrayListActorProxy = struct {
    pub const is_proxy = true;
    ctx: *Context,
    allocator: std.mem.Allocator,
    underlying: *ArrayListActor,
    
    const Self = @This();

    pub const Method = enum(u32) {
        handleStructWithArrayList = 0,
    };

    pub fn init(ctx: *Context, allocator: std.mem.Allocator) !*Self {
        const self = try allocator.create(Self);
        const underlying = try ArrayListActor.init(ctx, allocator);
        self.* = .{
            .ctx = ctx,
            .allocator = allocator,
    ...
    inline fn methodWrapper0(self: *Self, params: []const u8) !void {
        const result = try zborParse(StructWithArrayList, try zborDataItem.new(params), .{ .allocator = self.allocator });
        return self.underlying.handleStructWithArrayList(result);
    }

    pub inline fn handleStructWithArrayList(self: *Self, message: StructWithArrayList) !void {
