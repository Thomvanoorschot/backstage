...
const PubSubActor = @import("../pub_sub.zig").PubSubActor;

pub const PubSubActorProxy = struct {
    pub const is_proxy = true;
    ctx: *Context,
    allocator: std.mem.Allocator,
    underlying: *PubSubActor,
    
    const Self = @This();

    pub fn init(ctx: *Context, allocator: std.mem.Allocator) !*Self {
        const self = try allocator.create(Self);
        const underlying = try PubSubActor.init(ctx, allocator);
        self.* = .{
            .ctx = ctx,
            .allocator = allocator,
    ...
    fn methodWrapper0(self: *Self, params_json: []const u8) !void {
        ...
        defer params.deinit();
        try self.underlying.publish(params.value.message);
    }
